...
register_sidebar(array( // регистрируем левую колонку, этот кусок можно повторять для добавления новых областей для виджитов
	'name' => 'Сайдбар левый', // Название в админке
	'id' => "sidebar_left", // идентификатор для вызова в шаблонах
	'description' => 'Левая колонка', // Описалово в админке
	'before_widget' => '<div id="%1$s" class="widget %2$s"><div class="widget_head"></div>', // разметка до вывода каждого виджета
...
register_sidebar(array( // регистрируем левую колонку, этот кусок можно повторять для добавления новых областей для виджитов
	'name' => 'Сайдбар правый', // Название в админке
	'id' => "sidebar", // идентификатор для вызова в шаблонах
	'description' => 'Правая колонка', // Описалово в админке
	'before_widget' => '<div id="%1$s" class="widget %2$s"><div class="widget_head"></div>', // разметка до вывода каждого виджета
	'after_widget' => "</div>\n", // разметка после вывода каждого виджета
	'before_title' => '<span class="widgettitle">', //  разметка до вывода заголовка виджета
	'after_title' => "</span>\n", //  разметка после вывода заголовка виджета
...
/* в футер: */
register_sidebar(array( // регистрируем левую колонку, этот кусок можно повторять для добавления новых областей для виджитов
	'name' => 'Футер левый', // Название в админке
	'id' => "footer_sidebar_left", // идентификатор для вызова в шаблонах
	'description' => 'левая колонка футера', // Описалово в админке
	'before_widget' => '<div id="%1$s" class="footer_widget %2$s">', // разметка до вывода каждого виджета
	'after_widget' => "</div>\n", // разметка после вывода каждого виджета
	'before_title' => '<span class="widgettitle">', //  разметка до вывода заголовка виджета
	'after_title' => "</span>\n", //  разметка после вывода заголовка виджета
));
...
register_sidebar(array( // регистрируем левую колонку, этот кусок можно повторять для добавления новых областей для виджитов
	'name' => 'Футер центральный', // Название в админке
	'id' => "footer_sidebar_center", // идентификатор для вызова в шаблонах
	'description' => 'центральная колонка футера', // Описалово в админке
	'before_widget' => '<div id="%1$s" class="footer_widget %2$s">', // разметка до вывода каждого виджета
	'after_widget' => "</div>\n", // разметка после вывода каждого виджета
	'before_title' => '<span class="widgettitle">', //  разметка до вывода заголовка виджета
	'after_title' => "</span>\n", //  разметка после вывода заголовка виджета
...
register_sidebar(array( // регистрируем левую колонку, этот кусок можно повторять для добавления новых областей для виджитов
	'name' => 'Футер правый', // Название в админке
	'id' => "footer_sidebar_right", // идентификатор для вызова в шаблонах
	'description' => 'Правая колонка футера', // Описалово в админке
	'before_widget' => '<div id="%1$s" class="footer_widget %2$s">', // разметка до вывода каждого виджета
	'after_widget' => "</div>\n", // разметка после вывода каждого виджета
	'before_title' => '<span class="widgettitle">', //  разметка до вывода заголовка виджета
	'after_title' => "</span>\n", //  разметка после вывода заголовка виджета
...
if (!function_exists('content_class_by_sidebar')) { // если ф-я уже есть в дочерней теме - нам не надо её определять
	function content_class_by_sidebar() { // функция для вывода класса в зависимости от существования виджетов в сайдбаре
		/*if (is_active_sidebar( 'sidebar' )) { // если есть
			echo 'center_block'; // col-sm-6 - пишем класс на 80% ширины
		} else { // если нет
			echo 'center_block'; // col-sm-12 - контент на всю ширину
		}*/
	}
}


function check_tag ($post_id, $search_tag) {  
	/* 
	Функция принимает 2 значения - id поста и искомый тег(метка). Определяет, 
	нужно ли вывести надпись "План зала" под постом афиши.
	Если нет метки (которая указывается дальше по коду), то функция возращает
	свойство "hidden", которое скрывает тег. Если же метка есть, то ничего
	не вызращает, и тег отображается.
	*/
	$key = 1;
	$my_tmp = '';
	$posttags = get_the_tags($post_id); /* получени меток поста */
	if ($posttags) {
		foreach($posttags as $tag) {
			$my_tmp = $tag->name; 
			if($my_tmp == $search_tag){$key = 0;} /* вместо test ввести метку, по которой нужно вывести надпись "План зала" */
		}
	}
...
function check_tag_2 ($post_id, $search_tag) {  
	/* 
	Функция принимает 2 значения - id поста и искомый тег(метка). Определяет, 
	нужно ли вывести надпись "План зала" под постом афиши.
	Если нет метки (которая указывается дальше по коду), то функция возращает
	...
	*/
	...
	if($key)
	{
		echo "<div class=\"link_btm posters_btm popmake-251\" ><center>Вход<br>свободный</center></div>";
		return;
	}
	else
	{
		echo "<div class=\"link_btm posters_btm popmake-251\" ><center>Забронировать<br> билет</center></div>";
		return;
	}
}
